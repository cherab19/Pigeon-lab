  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const { data: prof } = await dataClient.from("profiles").select("full_name, school_id").eq("user_id", user.id).single();
        if (prof) {
          setProfile(prof);
          if (prof.school_id) {
            const { data: school } = await dataClient.from("schools").select("name").eq("id", prof.school_id).single();
            if (school) setSchoolName(school.name);
          }
        }
        const { data: roles } = await dataClient.from("user_roles").select("role").eq("user_id", user.id);
        let resolvedRole: AppRole = "student";
        if (roles && roles.length > 0) {
          const priority: AppRole[] = ["super_admin", "school_admin", "teacher", "student"];
          const found = priority.find(r => roles.some((rd: { role: AppRole }) => rd.role === r));
      ...
      {userRole === "student" && (
        <>
          ...
          <Button variant="ghost" size="sm" className="text-muted-foreground justify-start" asChild>
            <Link href="/textbooks"><Library className="w-4 h-4 mr-1" /> {t("nav.library")}</Link>
          </Button>
          <Button variant="ghost" size="sm" className="text-muted-foreground justify-start" asChild>
            <Link href="/success-guide"><Sparkles className="w-4 h-4 mr-1" /> {t("nav.successGuide")}</Link>
          </Button>
        </>
      )}
      {isAdmin && (
        <>
          <Button variant="ghost" size="sm" className="text-muted-foreground justify-start" asChild>
            <Link href="/manage-users"><Users className="w-4 h-4 mr-1" /> {t("nav.members")}</Link>
          </Button>
          {userRole === "school_admin" && (
            <Button variant="ghost" size="sm" className="text-muted-foreground justify-start" asChild>
              <Link href="/subscribe"><Sparkles className="w-4 h-4 mr-1" /> {t("nav.subscription") || "Subscription"}</Link>
            </Button>
          )}
        </>
      )}
