...
export default function SuccessGuide() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [refRes, routineRes] = await Promise.all([
        supabase.from("student_reflections").select("*").eq("user_id", user.id).eq("week_of", weekOf).maybeSingle(),
        supabase.from("student_routines").select("schedule").eq("user_id", user.id).maybeSingle(),
      ]);
      if (refRes.data) {
        setReflection({ went_well: refRes.data.what_went_well || "", to_improve: refRes.data.what_to_improve || "" });
        setReflectionSaved(true);
      }
      if (routineRes.data?.schedule && Array.isArray(routineRes.data.schedule)) {
        setRoutine(routineRes.data.schedule as unknown as RoutineSlot[]);
      }
    };
    init();
  }, [navigate]);
  ...
  const saveReflection = async () => {
    const user = await getSafeUser();
    if (!user) return;
    if (!reflection.went_well.trim() && !reflection.to_improve.trim()) { toast.error(t("guide.reflectionEmpty")); return; }
    const weekOf = getWeekStart();
    const { error } = await supabase.from("student_reflections").upsert({
      user_id: user.id, week_of: weekOf,
      what_went_well: reflection.went_well, what_to_improve: reflection.to_improve,
  ...
  const xpForNextLevel = state ? Math.ceil((state.xp + 1) / 100) * 100 : 100;
      ...
      <main className="max-w-5xl mx-auto px-3 sm:px-6 py-8 sm:py-12 space-y-12 sm:space-y-20">
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center pb-10">
          ...
          <div className="flex justify-center gap-3 flex-wrap">
            <Button asChild><Link to="/lab">{t("guide.ctaLab")}</Link></Button>
            <Button asChild variant="outline"><Link to="/textbooks">{t("guide.ctaLibrary")}</Link></Button>
          </div>
        </motion.section>
      </main>
    </div>
